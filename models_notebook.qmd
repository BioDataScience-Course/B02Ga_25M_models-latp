...
Nos analyses révèlent la présence d'un dimorphisme sexuel significatif chez les genres *Emydocephalus* et *Aipysurus* se traduisant par des femelles ayant un corps plus large à longueur museau-cloaque égale (GLM), ainsi que des variations morphologiques au niveau de la tête et de la queue.

Les modèles les plus pertinents que nous retenons sont le modèle linéaire `snake2_lm` (R\^2 = 0.8) qui relie le diamètre de l’œil aux dimensions de la tête et au sexe ; et le modèle linéaire généralisé (GLM) `snake_glm1` qui permet de mettre en évidence l'impact du sexe sur la largeur corporelle (p\<0.001).

Ces résultats nous indiquent que les contraintes liées à la reproduction, comme le besoin d'un corps plus large chez les femelles pour la gestation, ont pu conduire à des adaptations morphologiques différentes entre les mâles et les femelles.

Ainsi, l'étude de ces traits morphologiques soutient l'idée d'une sélection sexuelle et naturelle agissant différemment sur les serpents marins mâles et femelles dans leur adaptation au milieu marin.
