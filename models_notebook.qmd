...
#GLM(M)1Lea-Amelie
#Test1
snake1_glm <- glm(data = sea_snake1,body_width ~ svl + sex + species,
  family = Gamma(link = "log"))
summary(snake1_glm) |>tabularise()

#Test2 
sn2<- glm(data = sea_snake1,body_width ~ svl + sex, family = Gamma(link = "log"))
summary(sn2) |>tabularise()
...
# Vérification rapide
table(sea_snake1$sex, sea_snake1$sex_bin)

# 2. GLM binomial (régression logistique)
sn3 <- glm(data = sea_snake1, sex_bin ~ svl + tail_length + body_width + tail_width_max + species, family = binomial(link = "logit"))
  summary(sn3)|> tabularise()
...
# GLM gaussien (équivalent à un lm)
sn4 <- glm(data = sea_snake1, body_width ~ svl + sex * species, family = gaussian)
  summary(sn4)|>tabularise()
```
