...
# 2. GLM binomial (régression logistique)
  snake_glm1 <- glm(data = sea_snake1, sex_bin ~svl + tail_length + body_width + tail_width_max + species, family = binomial)
  summary(snake_glm1) |> tabularise()

```

-   La déviance résiduelle (310.3) est bien inférieure à la déviance totale (512.4), ce qui montre que les variables morphologique et l'espèce expliquent une grande partie de la différence entre les sexes.

-   La dispersion vaut 0.86 alors que le paramètre de dispersion du modèle est fixé à 1. Nous sommes dans un cas de légère sousdispersion.

-   Toutes les variables sont significatives au seuil alpha de 5% SAUF l'ordonée à l'origine (p=0.229), speciesfuscus (p=0.411 et specieslaevis (p=0.981)

```{r}
# chartGLM(M)1Lea-Amelie
ggplot(data = sea_snake1, aes(x = svl, y = sex_bin, color = species)) +
  geom_jitter(height = 0.05, width = 0, alpha = 0.6) +
  geom_smooth(method = "glm", method.args = list(family = "binomial"), se = FALSE, size = 1) +
  labs(title = "Modélisation du dimorphisme sexuel chez les serpents marins",
    subtitle = "Probabilité d'être un mâle en fonction de la taille (SVL) et de l'espèce",
    x = "Longueur Museau-Cloaque (SVL) [cm]",
    y = "Sexe (0 = Femelle, 1 = Mâle)",color = "Espèce") 
...
#  geom_point() +
#  stat_smooth(method = "glm",method.args = list(faùily = binomial), formula = y ~ x, se = FALSE)
```

...interpretation

```{r}
#chart$residuals(snake_glm2) 
```
